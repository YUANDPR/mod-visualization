...
function openToCurrentTab(obj) {
    if (isScrollToTop) {
        $(obj).show().siblings('.ACC_iframe').hide();
    } else {
        $(obj).css({
            "visibility": "visible",
            "position": "static"
        }).siblings('.ACC_iframe').css({"visibility": "hidden", "position": "absolute"});
    }
}
...
$(function () {
    ...
    function menuItem() {
        ...
        $('.menuTab').each(function () {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.mainContent .ACC_iframe').each(function () {
                        if ($(this).data('id') == dataUrl) {
                            openToCurrentTab(this);
                            return false;
                        }
        ...
        if (flag) {
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe class="ACC_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-refresh="' + isRefresh + '" seamless></iframe>';
            if (isScrollToTop) {
                $('.mainContent').find('iframe.ACC_iframe').hide().parents('.mainContent').append(str1);
            } else {
                $('.mainContent').find('iframe.ACC_iframe').css({
                    "visibility": "hidden",
                    "position": "absolute"
    ...
    function refreshTab() {
        ...
        var target = $('.ACC_iframe[data-id="' + currentId + '"]');
    ...
    function tabCloseOther() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function () {
            $('.ACC_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    function tabCloseAll() {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function () {
            if (isScrollToTop) {
                $('.ACC_iframe[data-id="' + $(this).data('id') + '"]').show();
            } else {
                $('.ACC_iframe[data-id="' + $(this).data('id') + '"]').css({
                    "visibility": "visible",
                    "position": "static"
                });
            }
    ...
    $.contextMenu({
        ...
        items: {
            ...
            "full": {
                ...
                callback: function (key, opt) {
                    setActiveTab(this);
                    var target = $('.ACC_iframe[data-id="' + this.data('id') + '"]');
                    target.fullScreen(true);
                }
            },
            "refresh": {
