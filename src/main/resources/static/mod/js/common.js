...
var refreshItem = function () {
    ...
    var target = $('.ACC_iframe[data-id="' + currentId + '"]', topWindow);
...
var closeItem = function (dataId) {
    ...
    if ($.common.isNotEmpty(dataId)) {
        ...
        // 移除相应tab对应的内容区
        $('.mainContent .ACC_iframe[data-id="' + dataId + '"]', topWindow).remove();
        return;
    }
    ...
    if ($.common.isNotEmpty(panelUrl)) {
        ...
        $('.mainContent .ACC_iframe', topWindow).each(function () {
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="ACC_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" seamless></iframe>';
        if (isScrollToTop) {
            $('.mainContent', topWindow).find('iframe.ACC_iframe').hide().parents('.mainContent').append(str1);
        } else {
            $('.mainContent', topWindow).find('iframe.ACC_iframe').css({
                "visibility": "hidden",
                "position": "absolute"
...
function activeWindow() {
    ...
    return $('.ACC_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
}
